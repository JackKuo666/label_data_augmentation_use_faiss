...
class SimbertStrategy():
    ...
    def __init__(self, configs_enlarge_path="./config.enlarge_strategy.yaml"):
        self.configs_enlarge = yaml.safe_load(open(configs_enlarge_path))
    ...
    def build_faiss_index(self, unlabel_query_path, index_save_path):
        df_unlable = pd.read_csv(unlabel_query_path, delimiter="\t")
        if TESTING:
            df_unlable = df_unlable.head(10000)
        # 文本数据列，也就是 item
        print(df_unlable.columns)
        target_col = df_unlable.columns[2]
    ...
    def query_search_by_faiss(self, index_save_path, label_path, unlabel_query_path, enlarge_query_save_path):
        ...
        label_df = pd.read_csv(label_path, delimiter="\t") if isinstance(label_path, str) else label_path
        unlabel_df = pd.read_csv(unlabel_query_path, delimiter="\t") if isinstance(unlabel_query_path, str) else unlabel_query_path
        if TESTING:
        ...
        if len(sim_text_list)>0:
            ...
            df_sim_text[['sim_text', 'sim_value', 'name']].to_csv(enlarge_query_save_path, index=False)  # 注意：这里一般是使用label，我这里是为了构造相似句对儿，所以使用name字段
            print("enlarge data save in {} done".format(enlarge_query_save_path))
    ...
    def get_enlarge_df(self, index_save_path, label_query_path, unlabel_query_path, enlarge_data_path):
        ...
        if(not os.path.isfile(index_save_path)):
            ...
            print("查到已经有索引{}，直接加载索引，如需更新请删除此文件".format(index_save_path))
...
if __name__ == "__main__":
    ...
    label_query_path = 'data/name_100_line.csv'                           #有标签数据集路径
    unlabel_query_path = 'data/patent_name_0_100w.csv'
    index_save_path = 'data/unlable_dataset.csv.index'
    enlarge_query_save_path = 'data/enlarge.csv'
